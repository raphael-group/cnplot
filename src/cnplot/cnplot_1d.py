"""1D genome scatter: observed values along the genome, with expected overlays.
...
def _draw_prop_legend(
    ...
    Proportions belong to the group, not to the copy-number profile: several
    samples routinely share one clonal CN solution while mixing those clones in
    different amounts. Printing them against the shared profile would attribute
    one sample's mixture to all of them, so they go here instead.
...
def plot_scatter_1d_multisample(
    ...
    **profile_kwargs,
    ...
    Args:
        ...
        inter_group_hspace: Space between groups.
